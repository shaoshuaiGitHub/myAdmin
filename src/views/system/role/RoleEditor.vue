      ...
      <div class="ant-pro-grid-content">
        <div class="ant-page-header">
          <a-breadcrumb>
            ...
            <a-breadcrumb-item>修改角色</a-breadcrumb-item>
          </a-breadcrumb>
          <div class="ant-page-header-heading">
            <span class="ant-page-header-heading-title">修改角色</span>
          </div>
        </div>
      </div>
    </div>
...
<script>
import { rolePage, getAllResource, roleUpdate, getResourceById } from "api";
...
export default {
  ...
  methods: {
    rolePage() {
      let that = this;
      rolePage({}).then(response => {
        if (response.code) {
          let roleData = response.data.list.filter(
            item => that.$route.params.id == item.roleId
          )[0];
         
          that.form.setFieldsValue({
            name: roleData.name,
            descr: roleData.descr
          });
          getAllResource({}).then(res => {
            that.data = res.data.content;
            setTimeout(function() {
              that.form.setFieldsValue({
                resourceIds: roleData.resourceIds
              });
            }, 0);
          });
        }
    ...
    handleSubmit(e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          values.roleId = this.$route.params.id;
